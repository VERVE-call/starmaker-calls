...
const AGORA_APP_ID = '4117c3467bc24d89b349d0d4e6d9f3dc';
...
async function initAgoraClient() {
  ...
  client.setParameter('DISABLE_TURN_TLS', false);
  client.setParameter('FORCE_TURN', false);
  client.setParameter('WEBSOCKET_RETRY_TIMEOUT_HINT', 8000);
  return client;
}
...
export async function startCall(callerId, receiverId, callerNumericId, localContainer, remoteContainer, cbs) {
  ...
  try {
    ...
    const channelName = `ch_${callerNumericId}`;

    client.on('user-published', async (user, mediaType) => {
      await client.subscribe(user, mediaType);
      if (mediaType === 'video') {
        const remoteTrack = user.videoTrack;
        remoteTrack.play(remoteContainer);
    ...
    client.on('user-unpublished', (user, mediaType) => {
      if (mediaType === 'video') callbacks.onEnded?.();
    });

    client.on('user-left', () => callbacks.onEnded?.());

    await client.join(AGORA_APP_ID, channelName, null, callerNumericId);
    ...
    callDocUnsub = onSnapshot(callRef, (snap) => {
      if (!snap.exists()) { callbacks.onEnded?.(); return; }
      const data = snap.data();
      if (data.status === 'rejected') { cleanupCall(); callbacks.onEnded?.(); }
      if (data.status === 'ended') { cleanupCall(); callbacks.onEnded?.(); }
    });
...
export async function acceptCall(localContainer, remoteContainer, cbs) {
  ...
  try {
    ...
    const { channelName, callerNumericId, receiverId } = callData;
    ...
    const receiverNumericId = receiverData?.callingId || (Math.floor(Math.random() * 900000) + 100000);
    ...
    client.on('user-unpublished', () => callbacks.onEnded?.());
    client.on('user-left', () => callbacks.onEnded?.());

    await client.join(AGORA_APP_ID, channelName, null, receiverNumericId);
    ...
    callDocUnsub = onSnapshot(callRef, (s) => {
      if (!s.exists()) { callbacks.onEnded?.(); return; }
      if (s.data().status === 'ended') { cleanupCall(); callbacks.onEnded?.(); }
    });

    await deleteDoc(doc(db, 'calls', `incoming_${receiverId}`));
...
async function cleanupCall() {
  ...
  if (agoraState.localAudioTrack) { agoraState.localAudioTrack.stop(); agoraState.localAudioTrack.close(); agoraState.localAudioTrack = null; }
  if (agoraState.localVideoTrack) { agoraState.localVideoTrack.stop(); agoraState.localVideoTrack.close(); agoraState.localVideoTrack = null; }
